            ...
            function (data, status) {
                ...
                if (data == "null") {
                    ...
                    db.transaction(function (tx) {
                        ...
                        var query = "DELETE FROM orderHistory where VendorID = ?";

                        tx.executeSql(query, [vendoreinfo[counturl].input], function (tx, res) {
                        ...
                        db.transaction(function (tx) {
                            for (var i = 0; i < dataimp.length; i++) {
                                dataimp[i].VendorName = vendoreinfo[counturl].input;
                                // delete and insert into order history table
                                var ordersIs = JSON.stringify(dataimp[i].OrderItems);
                                Object.keys(dataimp[i].OrderItems)
                                    .forEach(function eachKey(key) {
                                        tx.executeSql('INSERT INTO orderHistory VALUES (?,?,?,?,?,?,?)', [dataimp[i].OrderDate, dataimp[i].OrderID, ordersIs, key, dataimp[i].OrderItems[key], dataimp[i].Status, vendoreinfo[counturl].input]);
                                        console.log(key); // alerts key 
                                        console.log(dataimp[i].OrderItems[key]); // alerts value
                                    });
                               // tx.executeSql('INSERT INTO orderHistory VALUES (?,?,?,?,?)', [dataimp[i].OrderDate, dataimp[i].OrderID, ordersIs, dataimp[i].Status, vendoreinfo[counturl].input]);
                            }
                            
                        }, function (error) {

                            console.log('Transaction orderHistory ERROR: ' + error.message);
                        }, function () {
                            console.log('Populated database orderHistory  OK');
                            // strat get items
